...
function rendezvousApproach {
  ...
  LOCAL function CancelVelT {
    uiDebug("Maneuver: Match transverse velocity").
    if velT:mag > 1 {
      local lock steerDir to lookdirup(-velT:normalized, ship:facing:upvector).
  ...
  LOCAL function CancelVel {
    uiDebug("Maneuver: Match velocity").
    if vel:mag > 1 {
      local lock steerDir to lookdirup(-vel:normalized, ship:facing:upvector).
      lock steering to steerDir.
      wait until utilIsShipFacing(steerDir:vector).
  ...
  LOCAL function GetCloser {
    uiDebug("Maneuver: Accelerate towards target").
    // Make sure the ship is approaching target
    local dot is vdot(target:position, velR).
    until dot >= 0 {
        local lock steerDir to lookdirup(target:position, ship:facing:upvector).
        lock steering to steerDir.
  ...
  LOCAL function BrakeForEncounter {
    //Turn back to brake 
    local lock steerDir to lookdirup(-vel:normalized, ship:facing:upvector).
    lock steering to steerDir.
    wait until utilIsShipFacing(steerDir:vector) .
    ...
    if dt > 0 {
      if dt > 60 {
        uiDebug("Maneuver: Warping to brake").
        warpSeconds(dt).
      }
      else {
        uiDebug("Maneuver: Waiting " + round(dt) + " seconds to brake").
        wait dt.
      }
    }

    uiDebug("Maneuver: Braking.").
